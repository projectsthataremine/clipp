...
const { getMachineInfo } = require('./machine-info');
...
function initAuthHandlers(settingsWindow) {
  ...
  ipcMain.handle('START_OAUTH', async (event, { provider }) => {
    try {
      ...
      return new Promise((resolve, reject) => {
        const server = http.createServer(async (req, res) => {
          ...
          if (req.url.startsWith('/?access_token=')) {
            try {
              ...
              // Send success page
              res.writeHead(200, { 'Content-Type': 'text/html' });
              res.end('<html><body><h1>Sign in successful!</h1><p>You can close this window and return to Clipp.</p></body></html>');
        ...
        // Start the server on port 54321
  ...
  ipcMain.handle('RENAME_MACHINE', async (event, { licenseId, newName }) => {
    try {
      ...
      // First, get the current license to access metadata
      ...
      if (currentLicense.metadata) {
        try {
          ...
          const metadataObj = typeof currentLicense.metadata === 'string'
            ? JSON.parse(currentLicense.metadata)
            : currentLicense.metadata;
          updatedMetadata = { ...metadataObj, machine_name: newName };
        } catch (e) {
          console.error('[Auth] Failed to parse metadata:', e);
          updatedMetadata = { machine_name: newName };
        }
      }
      ...
      throw new Error(error.message || 'Failed to rename machine');
    }
  });
 ...
 */
async function ensureUserHasTrial(userId) {
  if (!userId) return;
